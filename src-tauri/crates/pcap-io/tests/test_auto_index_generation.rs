...
fn test_auto_index_generation_on_write() {
    ...
    let dataset_name = "auto_index_write_test";
    ...
    let mut writer = PcapWriter::new(base_path, dataset_name).expect("创建Writer失败");
    ...
    assert!(
        check_pidx_file_exists(&dataset_path),
        "自动索引生成失败：PIDX文件不存在"
    );
...
fn test_auto_index_generation_on_read() {
    let temp_dir = TempDir::new().expect("创建临时目录失败");
    let base_path = temp_dir.path();
    let dataset_name = "auto_index_read_test";
...
fn test_manual_index_generation_after_write() {
    let temp_dir = TempDir::new().expect("创建临时目录失败");
    let base_path = temp_dir.path();
    let dataset_name = "manual_index_after_write";
    ...
    // 步骤3: 使用索引读取验证
    let reader = PcapReader::new(base_path, dataset_name).expect("使用索引创建Reader失败");
...
fn test_index_accuracy_verification() {
    let temp_dir = TempDir::new().expect("创建临时目录失败");
    let base_path = temp_dir.path();
    let dataset_name = "index_accuracy_test";
    ...
    assert!(
        index.start_timestamp <= *min_timestamp,
        "索引开始时间戳不准确"
    );
    assert!(
        index.end_timestamp >= *max_timestamp,
        "索引结束时间戳不准确"
    );
    ...
    assert_eq!(
        total_indexed_packets, PACKET_COUNT as u64,
        "索引文件中数据包总数不匹配"
    );

    // 使用索引读取并验证
    let mut reader = PcapReader::new(base_path, dataset_name).expect("使用索引创建Reader失败");
    ...
    assert_eq!(
        actual_read_count, PACKET_COUNT,
        "使用索引读取的数据包数量不正确"
    );
    assert_eq!(
...
fn test_index_performance_benefits() {
    let temp_dir = TempDir::new().expect("创建临时目录失败");
    let base_path = temp_dir.path();
    let dataset_name = "index_performance_test";
    ...
    let reader = PcapReader::new(base_path, dataset_name).expect("创建有索引的Reader失败");
    let dataset_info = reader.dataset_info();
    ...
    // 验证索引提供了快速的元数据访问
    assert!(
        indexed_info_time.as_millis() < 100,
        "使用索引获取数据集信息耗时过长：{}ms",
        indexed_info_time.as_millis()
    );

    println!(
        "✅ 索引性能优势测试通过：获取 {} 个数据包信息耗时 {}ms",
        PACKET_COUNT,
        indexed_info_time.as_millis()
    );
}
