...
impl ConfigManager {
    ...
    /// 根据数据集名称创建UDP发送器
    pub fn create_udp_sender_for_dataset(
        &self,
        dataset_name: &str,
    ) -> Result<UDPSender, String> {
        let config = self.config.get_dataset_config(dataset_name)
        ...
        let mode = match config.udp_config.mode.as_str() {
            ...
            "unicast" => {
                let addr: SocketAddr = format!("{}:{}", 
                    config.udp_config.target_ip, 
                    config.udp_config.target_port
                ).parse()
                .map_err(|_| format!("无效的目标地址: {}", 
                    format!("{}:{}", config.udp_config.target_ip, config.udp_config.target_port)))?;
        ...
        let sender = UDPSender::new(mode, target_addr)
            .map_err(|e| format!("创建UDP发送器失败: {:?}", e))?;

        info!("为数据集 '{}' 创建UDP发送器成功: {}", dataset_name, target_addr);
        Ok(sender)
    }
    ...
    /// 获取所有启用的数据集配置
    pub fn get_enabled_datasets(&self
    ) -> Vec<&DatasetConfigState> {
        self.config.list_dataset_configs()
            .into_iter()
